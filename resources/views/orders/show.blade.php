...
@section('content')
    ...
    <p>Size - {{ $order->size }}</p>
    @if ($order->size == 'Small')
        <p>Total Price: - ${{ $pizza->price + count($order->toppings) * 3 }}.00</p>
        <ul>
            <li>Pizza - ${{ $pizza->price }}</li>
            <li>Extra Toppings - ${{ count($order->toppings) * 3 }}.00</li>
        </ul>
    @else
        <p>Total Price: - ${{ $pizza->price + (count($order->toppings) * 3) + 10}}.00</p>
        <ul>
